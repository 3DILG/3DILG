...
def train_batch(model, surface, points, labels, criterion):
    ...
    outputs, centers, shifted, sigma, loss_inside = model(surface, points)

    loss_vol = criterion(outputs[:, :1024], labels[:, :1024])
    loss_near = criterion(outputs[:, 1024:], labels[:, 1024:])

    loss_pull = shifted.square().sum(dim=-1).mean()
    loss_center = centers.square().sum(dim=-1).mean()
    loss_sigma = sigma.mean()

    loss = loss_vol + 0.1 * loss_near + 0.0001 * loss_sigma 

    return loss, outputs, loss_vol.item(), loss_near.item(), loss_center.item(), loss_pull.item(), loss_sigma.item(), loss_inside.item()
    ...
    for data_iter_step, (points, labels, surface, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                loss, output, loss_vol, loss_near, loss_center, loss_pull, loss_sigma, loss_inside = train_batch(model, surface, points, labels, criterion)
        ...
        metric_logger.update(loss_pull=loss_pull)
        metric_logger.update(loss_center=loss_center)
        metric_logger.update(loss_sigma=loss_sigma)
        metric_logger.update(loss_inside=loss_inside)
...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 200, header):
        ...
        with torch.cuda.amp.autocast():
            N = 100000
            output = torch.cat([model(surface, points[:, i*N:(i+1)*N])[0] for i in range(math.ceil(points.shape[1]/N))], dim=1)
